...
### UInputNumber theming bug: root cause found and fixed

The gap was real and is now closed, not just documented. `#build/ui/input-number.ts` (Nuxt UI's generated base theme) declares `variants.size.md` as a plain class **string** (`'px-2.5 py-1.5 text-base/5 gap-1.5'`), unlike `input.ts`/`select.ts`, which declare it as an **object** keyed by slot (`{ base: '...', leading: '...' }`). The `inputNumber` app.config block used the object shape — the one that works for every other input-family component — which mismatched the shorthand this specific component's base theme actually uses, and that mismatch is what produced the stray `[object Object]` class token and silently dropped the size override.

Found by direct experiment, not more static reading: first ruled out `number/component.vue`'s own `:ui="controlUi"` binding by swapping it for `:ui="{}"` and confirming the bug was unchanged (reverted, no component file changes needed), which pointed the search at the appConfig shape itself; diffing `input-number.ts` against `input.ts`/`select.ts` found the string-vs-object mismatch directly. Switched `inputNumber.variants.size` to the string form and it was fixed immediately — verified via computed style (34px height, 13px text, no artifact) on both a standalone number field and one nested inside an `array-table`.

Every field kind audited this session against the Atelier scale is now confirmed working: `badge`, `button`, `checkbox`, `formField`, `input`, `inputMenu`, `inputNumber`, `inputTags`, `inputTime`, `select`, `selectMenu`, `switch`, `tabs`, `textarea`. `colorPicker`, `pinInput`, and `slider` have no maquette reference anywhere in the source (re-checked with a broad search, not just asserted) and are correctly left at Nuxt UI's defaults. Phase 5 has no more open items from this session's audit.
