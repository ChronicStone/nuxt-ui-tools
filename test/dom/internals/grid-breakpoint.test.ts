...
describe('grid geometry', () => {
  it('resolves responsive grid columns and chunks rows accordingly', async () => {
    ...
    const grid = harness.internals.grid
    ...
    expect(grid.rowChunks.value[0]).toMatchObject({ index: 0, start: 0, end: 3 })
...
describe('breakpoint composable', () => {
  const Probe = defineComponent({
    setup() {
      ...
      return () => h('div', { 'data-mobile': String(state.isMobile.value), 'data-tablet': String(state.isTablet.value) })
    },
  })

  it('reports mobile and tablet ranges from the viewport', async () => {
    harness = await mountLoaded({ schema: createAccountsSchema(), component: Probe, breakpoint: 'sm' })
