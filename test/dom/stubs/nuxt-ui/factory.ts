...
const optional = { type: null as unknown as PropType<unknown>, default: undefined }
...
function dataAttributes(props: Any, keys: string[]) {
  ...
  for (const key of keys) {
    ...
    out[`data-${key.replace(/[A-Z]/g, (c) => `-${c.toLowerCase()}`)}`] = value === '' ? 'true' : String(value)
  }
  return out
}
...
function uiAttributes(ui: unknown) {
  ...
  for (const [slot, value] of Object.entries(ui as Any)) {
    if (typeof value === 'string' && value) out[`data-ui-${slot.toLowerCase()}`] = value
  }
  return out
}

function flattenItems(items: unknown): Array<MenuItem & { group: number }> {
...
function renderItems(items: unknown) {
  ...
  return h(
    ...
    flat.map((item, index) =>
      item.type === 'label'
        ? h('span', { 'data-ui-item-label': '', class: item.class, key: index }, item.label)
        : item.type === 'separator'
  ...
  return defineComponent({
    ...
    setup(props, { slots, attrs }) {
      return () => {
        ...
        if (props.label != null) children.push(h('span', { 'data-ui-label': '' }, String(props.label)))
        if (slots.default) children.push(...(slots.default() ?? []))
        if (slots.trailing) children.push(h('span', { 'data-ui-slot': 'trailing' }, slots.trailing()))
        return h(
          tag,
          {
            ...attrs,
    ...
    setup(props, { slots, attrs, emit }) {
      return () => {
        ...
        if (open) {
          ...
          if (slots.header) content.push(h('div', { 'data-ui-slot': 'header' }, slots.header({ close: () => {} })))
          if (slots.content) content.push(h('div', { 'data-ui-slot': 'content' }, slots.content({ close: () => {} })))
          if (slots.body) content.push(h('div', { 'data-ui-slot': 'body' }, slots.body({ close: () => {} })))
          if (slots.footer) content.push(h('div', { 'data-ui-slot': 'footer' }, slots.footer({ close: () => {} })))
        }
...
export function createInputStub(name: string, kind: 'text' | 'checkbox' | 'number' | 'select' | 'radio') {
  return defineComponent({
    name,
    inheritAttrs: false,
    props: {
      ...
      return () => {
        ...
        if (kind === 'select') {
          ...
          return h(
            ...
            items.map((item) =>
              h('option', { value: String(item.value), selected: String(item.value) === String(props.modelValue) }, item.label),
            ),
          )
        }
        if (kind === 'radio') {
        ...
        return h('span', { 'data-ui-wrap': name, class: ui?.root }, [
          h('input', {
            ...
            onInput: (event: Event) => {
              ...
              emit('update:modelValue', kind === 'number' ? (value === '' ? undefined : Number(value)) : value)
            },
          }),
          slots.trailing?.(),
        ])
...
export const UPaginationStub = defineComponent({
  ...
  setup(props, { attrs, emit }) {
    return () => {
      ...
      const pages = Math.max(1, Math.ceil(Number(props.total ?? 0) / Math.max(1, Number(props.itemsPerPage ?? 1))))
      const ui = props.ui as Any | undefined
      return h(
        'nav',
        {
        ...
        [
          h('button', { type: 'button', 'data-ui-page-first': '', class: ui?.first, onClick: () => emit('update:page', 1) }, '«'),
          h('button', { type: 'button', 'data-ui-page-prev': '', class: ui?.prev, disabled: page <= 1, onClick: () => emit('update:page', page - 1) }, '‹'),
          ...Array.from({ length: pages }, (_, index) =>
            h('button', { type: 'button', 'data-ui-page': String(index + 1), 'data-active': String(index + 1 === page), class: ui?.item, onClick: () => emit('update:page', index + 1) }, String(index + 1)),
          ),
          h('button', { type: 'button', 'data-ui-page-next': '', class: ui?.next, disabled: page >= pages, onClick: () => emit('update:page', page + 1) }, '›'),
          h('button', { type: 'button', 'data-ui-page-last': '', class: ui?.last, onClick: () => emit('update:page', pages) }, '»'),
        ],
      )
    }
  },
