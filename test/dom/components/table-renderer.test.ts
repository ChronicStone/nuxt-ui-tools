...
import { mountDataList, mountLoaded, texts, type Harness } from '../harness'
...
async function mountTable(options: Parameters<typeof mountDataList>[0] & { tableProps?: Record<string, unknown> }) {
  const mounted = await mountLoaded({ ...options, render: () => h(TableRenderer, { height: '400px', ...options.tableProps }) })
  await mounted.until(() => mounted.wrapper.find('.nut-dl-table').attributes('data-loading') === 'false')
  return mounted
}
...
describe('TableRenderer skeleton and tokens', () => {
  it('renders typed skeleton rows while the first page loads', async () => {
    harness = await mountDataList({ schema: createAccountsSchema({ delay: 80 }), settle: false, render: () => h(TableRenderer, { height: '400px' }) })
    ...
    expect(first.find('td:nth-child(1) .nut-dl-skeleton.size-4').exists()).toBe(true)
    expect(first.find('td[class*="nut-dl-td"]:nth-child(2) .size-7').exists()).toBe(true)
    const cells = first.findAll('td')
    expect(cells[2]!.find('.rounded-full.size-\\[7px\\]').exists()).toBe(true)
    expect(cells[4]!.find('.nut-dl-skeleton.size-4.rounded-\\[4px\\]').exists()).toBe(true)
    expect(cells[5]!.find('.ml-auto').exists()).toBe(true)
    expect(cells.at(-1)!.text()).toBe('')
    expect(harness.wrapper.find('tfoot').exists()).toBe(false)
    expect(harness.wrapper.find('tr.nut-dl-row:not(.nut-dl-row--skeleton)').exists()).toBe(false)
  })
  ...
  it('exposes size tokens and gutter as CSS variables', async () => {
    ...
    harness = await mountTable({ schema: createAccountsSchema(), ui: { table: { gutter: 20, size: 'sm' } } })
    ...
    harness = await mountTable({ schema: createAccountsSchema(), tableProps: { size: 'lg', gutter: 8 } })
...
describe('TableRenderer structure', () => {
  it('renders headers, pinned seams, internal columns and body rows', async () => {
    ...
    expect(texts(w, '.nut-dl-th__label')).toEqual(['Nom', 'Statut', 'Pays', 'EDOF', 'Contrats', 'Conso.'])
    ...
    expect(select.find('input[type="checkbox"]').exists()).toBe(true)
    const name = w.find('th[data-col="name"]')
    expect(name.classes()).toEqual(expect.arrayContaining(['nut-dl-pin', 'nut-dl-pin--start', 'nut-dl-pin--last-start']))
    ...
    expect(w.find('th[data-col="status"] .nut-dl-th__sort').attributes('data-name')).toBe('i-lucide-chevrons-up-down')
    expect(w.find('th[data-col="name"] .nut-dl-rz').exists()).toBe(true)
    const actions = w.find('th[data-col="__row-actions"]')
    expect(actions.classes()).toEqual(expect.arrayContaining(['nut-dl-th--actions', 'nut-dl-pin--end', 'nut-dl-pin--first-end']))
    expect(actions.attributes('style')).toContain('right: 0px')

    const rows = w.findAll('tr.nut-dl-row:not(.nut-dl-row--skeleton)')
    expect(rows).toHaveLength(20)
    expect(rows[0]!.attributes('data-row-id')).toBe('acc-1')
    expect(rows[0]!.attributes('data-index')).toBe('0')
    expect(texts(w, 'td[data-col="name"] b.name').slice(0, 3)).toEqual(['Compte 001', 'Compte 002', 'Compte 003'])
    expect(texts(w, 'td[data-col="status"]').slice(0, 3)).toEqual(['Actif', 'En attente', 'Inactif'])
    ...
    expect((w.find('td[data-col="country"]').element as HTMLElement).style.getPropertyValue('--nut-dl-lines')).toBe('2')
    expect(w.find('td[data-col="__row-actions"]').classes()).toContain('nut-dl-td--actions')
    expect(w.find('td[data-col="__row-actions"] .nut-dl-rowbtn').attributes('data-icon')).toBe('i-lucide-ellipsis')
  ...
  it('reflects column visibility, ellipsis and ui slot classes', async () => {
    harness = await mountTable({
      ...
      tableProps: { ui: { wrapper: 'wrap-x', root: 'root-x', base: 'base-x', thead: 'thead-x', tbody: 'tbody-x', th: 'th-x', td: 'td-x', tr: 'tr-x', tfoot: 'tfoot-x' } },
    })
    const w = harness.wrapper
    ...
    expect(w.find('td[data-col="legalEntity"]').exists()).toBe(false)
  ...
  it('sorts from the header menu and marks the sorted header', async () => {
    ...
    expect(items.map((item) => item.text())).toEqual(['Trier A → Z', 'Trier Z → A', 'Ne plus trier', 'Épingler à gauche', 'Épingler à droite', 'Masquer la colonne'])
    ...
    expect(w.find('th[data-col="status"] .nut-dl-th__sort').attributes('data-name')).toBe('i-lucide-arrow-down')
    expect(w.find('th[data-col="name"] .nut-dl-th__btn').classes()).not.toContain('nut-dl-th__btn--sorted')
    expect(w.find('td[data-col="status"]').text()).toBe('En attente')

    await w.find('th[data-col="status"]').findAll('[data-ui-item]').at(-1)!.trigger('click')
    await harness.flush()
    expect(w.find('th[data-col="status"]').exists()).toBe(false)
  })
  ...
  it('toggles selection from header and row checkboxes', async () => {
    ...
    await harness.flush()
    expect(harness.internals.selection.selectedKeys.value).toEqual(['acc-1'])
  ...
  it('renders row action menus per row', async () => {
    ...
    expect(cells[0]!.findAll('[data-ui-item]').map((item) => item.text())).toEqual(['Voir la fiche'])
    expect(cells[1]!.findAll('[data-ui-item]').map((item) => item.text())).toEqual(['Voir la fiche', 'Activer'])
  })

  it('virtualizes long pages', async () => {
    harness = await mountTable({ schema: createAccountsSchema(), query: { 'p.size': '50' } })
    const root = harness.wrapper.find('.nut-dl-table')
    expect(root.attributes('data-virtualized')).toBe('true')
    expect(harness.wrapper.findAll('tr.nut-dl-row:not(.nut-dl-row--skeleton)').length).toBeLessThanOrEqual(50)
  })
})
...
describe('TableRenderer summaries', () => {
  it('renders the sticky summary row with label, count, derived and async cells', async () => {
    ...
    expect(foot.find('td[data-col="contracts"] .nut-dl-tf__value').text()).toBe(String(sum('contracts')))
    expect(foot.find('td[data-col="contracts"]').classes()).toContain('text-right')
    expect(foot.find('td[data-col="status"]').text()).toBe('')
    await harness.until(() => !harness!.internals.summaries.cell('consumption').loading)
    expect(w.find('tfoot td[data-col="consumption"] .nut-dl-tf__value').text()).toBe(`${sum('consumption')} t`)
  ...
  it('shows a skeleton while an async summary resolves and formats numbers per locale', async () => {
    ...
    if (column) column.summary = { resolve: () => new Promise<number>((resolve) => (release = resolve)) }
    harness = await mountTable({ schema })
    expect(harness.wrapper.find('tfoot td[data-col="consumption"] .nut-dl-tf__skeleton').exists()).toBe(true)
    release(1234567)
    await harness.until(() => harness!.wrapper.find('tfoot td[data-col="consumption"] .nut-dl-tf__value').exists())
    expect(harness.wrapper.find('tfoot td[data-col="consumption"] .nut-dl-tf__value').text()).toBe('1 234 567')
  })

  it('uses a custom label and render function', async () => {
    const schema = createAccountsSchema({ rows: rows60 })
    schema.table!.summaries = { scope: 'page', label: 'Somme' }
    const column = schema.table?.columns?.find((entry) => entry.key === 'contracts')
    if (column) column.summary = { kind: 'sum', render: ({ value, scope }) => h('em', { class: 'custom' }, `${String(value)}/${scope}`) }
    harness = await mountTable({ schema })
    ...
    expect(foot.find('td[data-col="contracts"] em.custom').text()).toBe(`${rows60.slice(0, 20).reduce((t, r) => t + r.contracts, 0)}/page`)
  })
})
...
describe('TableRenderer empty state', () => {
  it('renders the plain empty state without rows', async () => {
    ...
    expect(empty.find('.nut-dl-empty__actions').exists()).toBe(false)
    expect(harness.wrapper.find('tfoot').exists()).toBe(false)
    expect(harness.wrapper.find('.nut-dl-table').attributes('data-loading')).toBe('false')
  })
  ...
  it('renders the filtered empty state with a working reset action', async () => {
    ...
    await harness.until(() => harness!.wrapper.find('.nut-dl-empty').exists())
    const empty = harness.wrapper.find('.nut-dl-empty')
  ...
  it('honours empty state overrides from the ui config', async () => {
    harness = await mountTable({
      ...
      ui: { table: { props: { empty: { title: 'Vide', description: false, icon: 'i-lucide-ghost' } }, ui: { empty: 'empty-x' } } },
    })
    const empty = harness.wrapper.find('.nut-dl-empty')
    expect(empty.classes()).toContain('empty-x')
    expect(empty.find('.nut-dl-empty__title').text()).toBe('Vide')
    expect(empty.find('.nut-dl-empty__description').exists()).toBe(false)
...
describe('TableRenderer cursor mode', () => {
  it('auto-loads following pages when every row fits and shows a loading-more row meanwhile', async () => {
    harness = await mountTable({ schema: createAuditSchema({ total: 45, pageSize: 20, delay: 60 }) })
    await harness.until(() => harness!.wrapper.find('.nut-dl-row--loading-more').exists())
    expect(harness.wrapper.find('.nut-dl-loading-more').text()).toBe('Chargement…')
    expect(harness.wrapper.find('.nut-dl-loading-more .nut-dl-spinner').exists()).toBe(true)
    await harness.until(() => harness!.internals.pagination.loadedCount.value === 45, 4000)
    await harness.until(() => !harness!.wrapper.find('.nut-dl-row--loading-more').exists())
    expect(harness.wrapper.find('.nut-dl-table').attributes('data-virtualized')).toBe('true')
    expect(harness.internals.pagination.canNextPage.value).toBe(false)
  })
})
