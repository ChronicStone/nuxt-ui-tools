...
async function mountPanel(options: Partial<Parameters<typeof mountLoaded>[0]> & { panelProps?: Record<string, unknown>; open?: boolean } = {}) {
  const mounted = await mountLoaded({ schema: createAccountsSchema({ panelFilters: true }), ...options, render: () => h(DataListFilterPanel, options.panelProps) })
...
describe('filter slideover trigger', () => {
  it('renders the trigger with icon, label and an active count badge', async () => {
    ...
    expect(trigger.find('.nut-dl-fpanel-trigger__count').exists()).toBe(false)
    expect(harness.wrapper.find('[data-ui="USlideover"]').attributes('data-open')).toBe('false')

    harness.internals.filters.replaceFilters({ rules: [{ key: 'country', operator: 'isAnyOf', value: ['FR'] }] })
    await harness.flush()
  ...
  it('is not rendered without panel filters', async () => {
    harness = await mountLoaded({ schema: createAccountsSchema(), render: () => h(DataListFilterPanel) })
    expect(harness.wrapper.find('.nut-dl-fpanel-trigger').exists()).toBe(false)
  })
})
...
describe('filter slideover content', () => {
  it('opens with a title, result count, captioned sections and typed fields', async () => {
    ...
    expect(texts(w, '.nut-dl-fpanel__caption')).toEqual(['Identité', 'Volumes'])
    expect(texts(w, '.nut-dl-fpanel__label')).toEqual(['Pays', 'Entité légale', 'Contrats'])
    ...
    expect(w.find('input[data-ui="UInputNumber"]').exists()).toBe(true)
  })
  ...
  it('renders option filters as chips with counts and drafts until apply', async () => {
    ...
    expect(chips.map((chip) => chip.find('.nut-dl-chip__label').text())).toEqual(['FR', 'DE', 'ES'])
    expect(chips.map((chip) => chip.find('.nut-dl-chip__count').text())).toEqual(['20', '20', '20'])
    expect(chips[0]!.attributes('aria-pressed')).toBe('false')
    expect(w.find('.nut-dl-fpanel__meta').exists()).toBe(false)
    ...
    await harness.flush()
    expect(harness.internals.filterPresentation.panelOpen.value).toBe(false)
    expect(harness.internals.filters.getFilterState({ key: 'country' })?.value).toEqual(['FR', 'DE'])
  ...
  it('edits text and number fields and resets the draft', async () => {
    ...
    await harness.flush()
    expect(harness.internals.filterPresentation.getPanelDraftFilterState({ key: 'legalEntity' })).toMatchObject({ value: 'Entité 1', operator: 'contains' })
    await w.find('input[data-ui="UInputNumber"]').setValue('3')
    await harness.flush()
    expect(harness.internals.filterPresentation.getPanelDraftFilterState({ key: 'contracts' })).toMatchObject({ value: 3, operator: 'is' })
    await w.find('.nut-dl-fpanel__reset').trigger('click')
    await harness.flush()
    expect(harness.internals.filterPresentation.getPanelDraftFilterState({ key: 'legalEntity' })).toBeUndefined()
    expect(harness.internals.filterPresentation.getPanelDraftFilterState({ key: 'contracts' })).toBeUndefined()
  ...
  it('commits live and swaps the primary action to done', async () => {
    ...
    await harness.flush()
    expect(harness.internals.filters.getFilterState({ key: 'country' })?.value).toEqual(['ES'])
  ...
  it('closes from the header button and reopens with the committed state', async () => {
    ...
    await harness.flush()
    expect(harness.internals.filterPresentation.panelOpen.value).toBe(false)
    harness.internals.filters.replaceFilters({ rules: [{ key: 'country', operator: 'isAnyOf', value: ['DE'] }] })
    await harness.flush()
    await harness.wrapper.find('.nut-dl-fpanel-trigger').trigger('click')
    await harness.flush()
    expect(harness.internals.filterPresentation.panelOpen.value).toBe(true)
    expect(harness.wrapper.find('.nut-dl-chip[data-value="DE"]').classes()).toContain('nut-dl-chip--active')
  })

  it('renders inline in panel mode with the same footer', async () => {
    harness = await mountPanel({ panelProps: { mode: 'panel' }, open: false })
    const w = harness.wrapper
    expect(w.find('[data-ui="USlideover"]').exists()).toBe(false)
    expect(w.find('.nut-dl-fpanel--inline').exists()).toBe(true)
    expect(harness.internals.filterPresentation.panelOpen.value).toBe(true)
    expect(texts(w, '.nut-dl-fpanel__label')).toEqual(['Pays', 'Entité légale', 'Contrats'])
    expect(w.find('.nut-dl-fpanel__apply').exists()).toBe(true)
    harness.unmount()
    harness = await mountPanel({ panelProps: { mode: 'panel', commitMode: 'live' }, open: false })
    expect(harness.wrapper.find('.nut-dl-fpanel__apply').exists()).toBe(false)
    expect(harness.wrapper.find('.nut-dl-fpanel__matching').exists()).toBe(true)
  })
  ...
  it('applies props and ui layers, including the chip threshold', async () => {
    harness = await mountPanel({
      ui: {
        filterPanel: {
          size: 'sm',
          props: { trigger: { variant: 'soft', label: 'Affiner' }, count: false, apply: { color: 'neutral' }, chips: 2 },
          ui: { content: 'content-x', chip: 'chip-x', sectionTitle: 'cap-x', field: 'field-x' },
        },
      },
      panelProps: { description: 'Affinez la liste.', ui: { title: 'title-x' }, props: { clear: { variant: 'link' } } },
    })
    const w = harness.wrapper
    ...
    expect(w.find('.nut-dl-chip').exists()).toBe(false)
    expect(w.find('[data-ui="UPopover"] [data-ui="UButton"]').exists()).toBe(true)
    ...
    harness.internals.filters.replaceFilters({ rules: [{ key: 'country', operator: 'isAnyOf', value: ['FR'] }] })
    await harness.flush()
    expect(w.find('.nut-dl-fpanel-trigger__count').exists()).toBe(false)
  })
  ...
  it('hides counts when no facet source backs the options', async () => {
    ...
    Object.assign(country, { source: { options: (country as unknown as { source: { options: unknown[] } }).source.options } })
    harness = await mountPanel({ schema })
    expect(harness.wrapper.findAll('.nut-dl-chip')).toHaveLength(3)
    expect(harness.wrapper.find('.nut-dl-chip__count').exists()).toBe(false)
  })

  it('renders boolean filters as two chips', async () => {
    const schema = createAccountsSchema({ panelFilters: true })
    const edof = schema.filters!.ui!.find((definition) => definition.key === 'edofSync')!
    edof.display = { location: 'panel', order: 6, group: 'Volumes' }
    ...
    expect(chips.map((chip) => chip.find('.nut-dl-chip__label').text())).toEqual(['Oui', 'Non'])
    expect(chips.map((chip) => chip.find('.nut-dl-chip__count').text())).toEqual(['30', '30'])
    await chips[0]!.trigger('click')
    await harness.flush()
    expect(harness.internals.filterPresentation.getPanelDraftFilterState({ key: 'edofSync' })?.value).toBe(true)
    ...
    await harness.flush()
    expect(harness.internals.filterPresentation.getPanelDraftFilterState({ key: 'edofSync' })).toBeUndefined()
  })
})
