...
describe('DataListSortMenu desktop', () => {
  ...
  it('is hidden for layouts it does not cover and uses grid sort options in grid mode', async () => {
    ...
    await harness.flush()
    await harness.wrapper.find('.nut-dl-sortbtn').trigger('click')
    await harness.flush()
    expect(harness.wrapper.find('.nut-dl-sortbtn').exists()).toBeTruthy()
    expect(
      harness.wrapper.findAll('.nut-dl-sort__row').map((row) => row.find('.flex-1').text()),
    ).toStrictEqual(['Nom', 'Statut'])
  })
...
describe('DataListSortMenu mobile sheet', () => {
  it('renders a square trigger and a bottom sheet with keys and order', async () => {
    ...
    const rows = w.findAll('.nut-dl-sort__row')
    ...
    const order = w.findAll('.nut-dl-sort__dir button')
    ...
    expect(must(w.findAll('.nut-dl-sort__dir button')[1]).classes()).toContain('bg-default')
  })
})
