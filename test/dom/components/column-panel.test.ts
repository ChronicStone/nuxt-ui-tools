...
async function mountPanel(options: Partial<Parameters<typeof mountLoaded>[0]> & { panelProps?: Record<string, unknown> } = {}) {
  const mounted = await mountLoaded({ schema: createAccountsSchema(), ...options, render: () => h(DataListColumnPanel, options.panelProps) })
...
describe('ColumnPanel', () => {
  ...
  it('lists configurable columns with pinned rows and toggles visibility', async () => {
    ...
    expect(w.find('.nut-dl-colpanel__title').text().replace(/\s+/g, ' ')).toBe('Colonnes · 6 sur 7')
    expect(w.find('.nut-dl-colpanel input[data-ui="UInput"]').exists()).toBe(false)
    const rows = w.findAll('.nut-dl-colpanel__row')
    expect(rows.map((row) => row.find('span.truncate').text())).toEqual(['Nom', 'Statut', 'Pays', 'Entité légale', 'EDOF', 'Contrats', 'Conso.'])
    expect(rows[0]!.find('input[type="checkbox"]').attributes('disabled')).toBeDefined()
    expect(rows[0]!.find('[data-ui="UIcon"][data-name="i-lucide-pin"]').exists()).toBe(true)
    expect(rows[1]!.find('.column-drag-handle').exists()).toBe(true)
    ...
    await harness.flush()
    expect(harness.internals.tableColumns.visibleOrderedColumns.value.map((c) => c.id)).not.toContain('status')
    ...
    await harness.flush()
    expect(harness.internals.tableColumns.visibleOrderedColumns.value.map((c) => c.id)).toContain('legalEntity')
  })

  it('filters rows by search and resets to defaults from the footer', async () => {
    harness = await mountPanel({ ui: { columnPanel: { props: { search: true, count: false, reset: { color: 'primary' } }, ui: { panel: 'panel-x', row: 'row-x' } } } })
    const w = harness.wrapper
    ...
    await harness.flush()
    expect(texts(w, '.nut-dl-colpanel__row span.truncate')).toEqual(['Pays'])
    ...
    await harness.flush()
    expect(harness.internals.tableColumns.visibleOrderedColumns.value.map((c) => c.id)).toContain('country')
  ...
  it('exposes trigger state through its slot', async () => {
    harness = await mountLoaded({
      ...
      render: () => h(DataListColumnPanel, null, { trigger: (scope: { openState: boolean; toggle: () => void }) => h('button', { class: 'custom-trigger', onClick: scope.toggle }, String(scope.openState)) }),
    })
