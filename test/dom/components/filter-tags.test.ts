...
const mountTags = (options: Partial<Parameters<typeof mountLoaded>[0]> & { tagProps?: Record<string, unknown> } = {}) =>
  mountLoaded({ schema: createAccountsSchema(), ...options, render: () => h(DataListFilterTags, { showAdd: true, showClear: true, ...options.tagProps }) })
...
describe('filter tags bar', () => {
  it('renders dormant tags dashed and the add-filter trigger', async () => {
    ...
    expect(dormant.find('[data-ui="UIcon"]').exists()).toBe(true)
    expect(w.find('.nut-dl-tag--active').exists()).toBe(false)
    const add = w.find('.nut-dl-tag--add')
    expect(add.attributes('data-label')).toBe('Ajouter un filtre')
    expect(add.attributes('data-icon')).toBe('i-lucide-plus')
    expect(add.classes()).toContain('border-dashed')
    expect(w.find('.nut-dl-tag--clear').exists()).toBe(false)
  })

  it('renders active tags as plain text entries with colour dots', async () => {
    harness = await mountTags({ schema: createAccountsSchema({ statusDefault: ['active', 'pending'] }) })
    const w = harness.wrapper
    const active = w.find('.nut-dl-tag--active')
    expect(active.exists()).toBe(true)
    expect(active.find('.nut-dl-tag__label').text()).toBe('Statut')
    expect(texts(w, '.nut-dl-tag__text')).toEqual(['Actif', 'En attente'])
    const dots = w.findAll('.nut-dl-tag__dot')
    expect(dots).toHaveLength(2)
    expect(dots[0]!.attributes('style')).toContain(STATUS_COLOR.active)
    expect(w.find('.nut-dl-tag__sep').exists()).toBe(false)
    expect(w.find('[data-ui="UBadge"]').exists()).toBe(false)
    const dismiss = w.find('.nut-dl-tag__dismiss')
    expect(dismiss.attributes('data-icon')).toBe('i-lucide-chevron-down')
    expect(dismiss.attributes('aria-label')).toBe('Statut')
    expect(w.find('.nut-dl-tag--clear').exists()).toBe(false)
  })
  ...
  it('promotes dynamic filters into removable tags and offers a reset', async () => {
    ...
    const w = harness.wrapper
    const active = w.find('.nut-dl-tag--active')
    expect(active.find('.nut-dl-tag__label').text()).toBe('Pays')
    expect(texts(active, '.nut-dl-tag__text')).toEqual(['FR', 'DE'])
    expect(active.findAll('.nut-dl-tag__sep')).toHaveLength(1)
    expect(active.find('[data-label="parmi"]').exists()).toBe(true)
    expect(active.find('[data-label="parmi"]').attributes('data-trailing-icon')).toBe('i-lucide-chevron-down')
    const dismiss = active.find('.nut-dl-tag__dismiss')
    expect(dismiss.attributes('data-icon')).toBe('i-lucide-x')
    expect(w.find('.nut-dl-tag--clear').attributes('data-icon')).toBe('i-lucide-rotate-ccw')
    expect(w.find('.nut-dl-tag--clear').text()).toBe('Réinitialiser')
    ...
    await harness.flush()
    expect(w.find('.nut-dl-tag--clear').exists()).toBe(true)
  ...
  it('walks the add-filter picker through match mode into an embedded editor', async () => {
    ...
    expect(w.find('input[data-ui="UInput"]').exists()).toBe(false)
    const rows = w.findAll('[data-filter-stage-content] button.rounded-md')
    expect(rows.map((row) => row.text())).toEqual(['Pays', 'Synchronisation EDOF'])
    expect(rows[0]!.find('[data-ui="UIcon"]').attributes('data-name')).toBe('i-lucide-plus')
    expect(rows[0]!.findAll('[data-ui="UIcon"]').at(-1)!.attributes('data-name')).toBe('i-lucide-chevron-right')
    ...
    expect(operators.map((b) => b.text().replace(/\s+/g, ''))).toEqual(['parmiin', 'est=', "n'estpas≠"])
    ...
    expect(head.find('.nut-dl-editor__back').exists()).toBe(true)
  ...
  it('applies filter tag props and add-filter picker options from the config layer', async () => {
    harness = await mountTags({
      ...
      ui: {
        filterTags: { props: { icon: false, trigger: { variant: 'outline' } }, ui: { activeRoot: 'active-x', value: 'value-x', addTrigger: 'add-x' } },
        addFilter: { props: { trigger: { label: 'Filtre' }, search: true, title: false, icon: 'kind' } },
      },
    })
    const w = harness.wrapper
    expect(w.find('.nut-dl-tag--active').classes()).toContain('active-x')
    expect(w.find('.nut-dl-tag__value').classes()).toContain('value-x')
    expect(w.find('.nut-dl-tag__label [data-ui="UIcon"]').exists()).toBe(false)
    const add = w.find('.nut-dl-tag--add')
    expect(add.attributes('data-label')).toBe('Filtre')
    expect(add.classes()).toContain('add-x')
    await add.trigger('click')
    await harness.flush()
    expect(w.find('.nut-dl-picker__title').exists()).toBe(false)
    expect(w.find('[data-filter-stage-content] input[data-ui="UInput"]').attributes('placeholder')).toBe('Rechercher des filtres...')
    const rowIcons = w.findAll('[data-filter-stage-content] button.rounded-md [data-ui="UIcon"]:first-child')
    ...
    await harness.flush()
    expect(texts(w, '[data-filter-stage-content] button.rounded-md')).toEqual(['Synchronisation EDOF'])
  })
  ...
  it('renders nothing when no tag or dynamic filters exist', async () => {
    ...
    expect(harness.wrapper.find('.nut-dl-tags').exists()).toBe(false)
  })
})

describe('mobile filter sheet', () => {
  it('replaces the tags with a bottom sheet listing every filter', async () => {
    harness = await mountTags({ breakpoint: 'sm', schema: createAccountsSchema({ statusDefault: ['active'] }) })
    const w = harness.wrapper
    expect(w.find('.nut-dl-tag--dormant').exists()).toBe(false)
    ...
    expect(trigger.find('[data-ui="UBadge"]').exists()).toBe(false)
    ...
    expect(rows.map((row) => row.find('.flex-1').text())).toEqual(['Statut', 'Pays', 'Synchronisation EDOF'])
    expect(rows[0]!.find('.nut-dl-sheet__value').text()).toContain('Actif')
    expect(rows[0]!.find('.nut-dl-sheet__value .rounded-full').attributes('style')).toContain(STATUS_COLOR.active)
    ...
    expect(detail.find('.nut-dl-editor__head').exists()).toBe(false)
    await detail.find('[data-ui="UButton"][data-icon="i-lucide-arrow-left"]').trigger('click')
    await harness.flush()
    expect(w.find('.nut-dl-sheet__detail').exists()).toBe(false)
  })
  ...
  it('shows the active count and clears everything from the footer', async () => {
    ...
    await harness.flush()
    expect(harness.internals.filters.hasActiveUiFilters.value).toBe(false)
    expect(harness.internals.filters.searchQuery.value).toBe('')
  })

  it('keeps inline tags on mobile when asked', async () => {
    harness = await mountTags({ breakpoint: 'sm', tagProps: { mobile: 'tags' } })
    expect(harness.wrapper.find('.nut-dl-sheet-trigger').exists()).toBe(false)
    expect(harness.wrapper.find('.nut-dl-tag--dormant').exists()).toBe(true)
  })
})
