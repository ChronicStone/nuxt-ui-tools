...
describe('data-list UI', () => {
  ...
  it('types root defaults for every granular rendering concern', () => {
    const ui: DataListUiConfig = {
      ...
      table: { ui: { wrapper: 'overflow-auto', root: 'min-w-full', th: 'h-8', td: 'py-1.5' } },
      ...
      mergeDataListUiConfig(
        {
          ...
          filterTags: {
            size: 'md',
            ui: { trigger: 'rounded-md', popoverContent: 'w-72', option: 'px-3' },
          },
        },
        {
          filterTags: { size: 'sm', ui: { trigger: 'rounded-sm', optionLabel: 'font-medium' } },
        },
        'compact',
      ),
    ).toMatchObject({
