...
import { resolvePanelCommitRules, resolvePanelDefaultRules } from '#ui-tools/table/utils/filter-presentation'
import { resolveFilterDisplayLocation, resolveFilterDisplayOrder } from '#ui-tools/table/utils/filters/presentation'
...
describe('panel rule resolution', () => {
  it('collects default rules for panel definitions only', () => {
    ...
    expect(rules).toEqual([{ key: 'a', value: ['x'] }])
    expect(rules[0]).not.toBe(rules[0] && { key: 'a', value: ['x'] })
  })

  it('replaces panel rules while preserving tag rules', () => {
    const current = [{ key: 'tag', value: 1 }, { key: 'panel', value: 'old' }]
    const next = resolvePanelCommitRules({ currentRules: current, panelKeys: ['panel', 'other'], panelRules: [{ key: 'other', value: 'new' }] })
    expect(next).toEqual([{ key: 'tag', value: 1 }, { key: 'other', value: 'new' }])
    expect(next[0]).toBe(current[0])
  })
})

describe('display resolution', () => {
  const definition = (display?: TableUiFilterDefinition['display']) => ({ kind: 'text', key: 'k', label: 'K', display }) as unknown as TableUiFilterDefinition

  it('defaults to tag, keeps valid locations and rejects unknown ones', () => {
    expect(resolveFilterDisplayLocation(undefined)).toBe('tag')
