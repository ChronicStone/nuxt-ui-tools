...
export type DecrementDepth<N extends number> = BuildTuple<N> extends [any, ...infer Rest]
  ? Rest['length']
  : 0
...
export type NestedPathsForType<
  T,
  P,
  ...
  = T extends Array<infer U>
    ...
    : {
        [K in keyof T as K extends `${string}.${string}`
          ? never
          : K]: T[K] extends object ? RemoveDotKeys<T[K]> : T[K];
      }
...
export type DeepPick<T, K extends string> = UnionToIntersection<K extends K
...
export type DeepTransform<T, FromType, ToType>
  = T extends FromType ? ToType
    : T extends Array<infer U> ? Array<DeepTransform<U, FromType, ToType>>
...
export type DeepRemoveIndexSignature<T> = T extends object
  ? T extends Array<infer U>
    ...
    : {
        [K in keyof T as string extends K ? never : number extends K ? never : K]: DeepRemoveIndexSignature<T[K]>
      }
  : T
