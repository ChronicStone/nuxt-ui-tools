import { isArray, isObject } from '@nuxt-ui-tools/shared'
...
export function getFilterRuleValue(options: {
  rule?: TableQueryStateFilterRule
}) {
  return options.rule?.value
}

export function getFilterPathValues(options: {
  source: unknown
  key: string
}): unknown[] {
...
export function readFilterPathValue(options: {
  source: unknown
  key: string
}): unknown {
...
export function flattenFilterValues(options: {
  value: unknown
}): unknown[] {
...
export function isDateRangeValue(options: {
  value: unknown
}): options is { value: DateRangeLike } {
...
export function formatFilterDate(options: {
  value: Date
}) {
...
export function isFilterValueSelected(options: {
  values: unknown[]
  candidate: unknown
}) {
  return options.values.some((value) => areFilterValuesEqual({ left: value, right: options.candidate }))
}

export function getDateRangeValue(options: {
  value: unknown
}) {
...
function areFilterValuesEqual(options: {
  left: unknown
  right: unknown
}) {
