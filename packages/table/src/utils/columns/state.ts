      ...
      left: uniqueColumnIds({
        columnIds: [
          ...
          ...options.runtimeColumns
            .filter(
              (column) =>
                findSchemaColumn({ schema: options.schema, columnId: column.id })?.pinned === 'left',
            )
            .map((column) => column.id),
        ],
      }),
...
function sanitizeColumnPinning(options: {
  ...
  }).filter((columnId) => columnId === SELECT_COLUMN_ID || options.visibleColumnIds.includes(columnId))
  ...
  }).filter((columnId) => options.visibleColumnIds.includes(columnId) && !pinnedLeft.includes(columnId))
