...
import { createResolvedFilterState } from '../src/utils'
import type { TableFilterResolveContext, TableResolvedFilterNode } from '../src'
      ...
      definitions: [
        {
          ...
          resolve({
            rule,
          }: TableFilterResolveContext) {
            return {
              type: 'group' as const,
              combinator: 'or' as const,
              children: (Array.isArray(rule.value) ? rule.value : []).map((value): TableResolvedFilterNode<string> => ({
