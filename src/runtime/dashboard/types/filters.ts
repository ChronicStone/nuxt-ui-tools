 ...
 * `<USelectMenu v-model="dashboard.filters.account.value" v-bind="dashboard.filters.account.menu" />`.
 */
...
/** A preset as the filter handle exposes it: resolved text, and whether the value matches it. */
export interface DashboardFilterPreset<TValue = unknown> {
  label: string
  value: TValue
  icon?: string
  hint?: string
...
/**
 * Everything a control needs to drive one param: its value, how to present it, its option list,
 ...
 */
export interface DashboardFilterHandle<
  TValue = unknown,
...
> {
  ...
  /** Menu columns declared on the param. */
  ...
  readonly presets: readonly DashboardFilterPreset<TValue>[]
...
/** Value a filter picks from its list: its value, or its element type for `multiple` params. */
type DashboardFilterItem<TParam> = TParam extends { readonly defaultValue: infer TValue }
  ? Extract<
...
type DashboardFilterValue<TParam> = TParam extends { readonly defaultValue: infer TValue }
  ? TValue
  : never

/** Filter handles of a params map, by param key. */
export type DashboardFilterHandles<TParams> = {
  readonly [K in keyof TParams]: DashboardFilterHandle<
    DashboardFilterValue<DashboardParamOf<TParams[K]>>,
    DashboardFilterItem<DashboardParamOf<TParams[K]>>
  >
}

/** What a filter bar drives: a dashboard (the root and its current view) or one view handle. */
export interface DashboardFiltersTarget {
  readonly filters: Readonly<Record<string, DashboardFilterHandle>>
