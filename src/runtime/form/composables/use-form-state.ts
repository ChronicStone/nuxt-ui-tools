...
export function useFormState(params: {
  ...
  const output = computed(() => buildFormOutput(params.schema.value, state, params.context, params.apiFactory))
  ...
  return {
    ...
    setValue: (path: string | readonly string[], value: unknown) => setPathValue(state, path, value),
  }
}
...
function collectDirtyPaths(initial: unknown, current: unknown, path: readonly string[] = []): readonly string[] {
  if (Array.isArray(initial) || Array.isArray(current))
    return collectArrayDirtyPaths(Array.isArray(initial) ? initial : [], Array.isArray(current) ? current : [], path)
  ...
  if (isRecord(initial) || isRecord(current)) {
    ...
    return [...keys].flatMap(key => collectDirtyPaths(initialObject[key], currentObject[key], [...path, key]))
  }

  return Object.is(initial, current) ? [] : [path.join('.')]
}

function collectArrayDirtyPaths(initial: readonly unknown[], current: readonly unknown[], path: readonly string[]) {
  if (initial.length !== current.length) return [path.join('.')]

  return current.flatMap((value, index) => collectDirtyPaths(initial[index], value, [...path, String(index)]))
}
