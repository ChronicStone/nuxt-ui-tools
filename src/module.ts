...
const cookieEsmPath = join(
  dirname(require.resolve("cookiejs/package.json")),
  "dist/cookie.esm.js",
);
...
export default defineNuxtModule<ModuleOptions>({
  ...
  async setup(options, nuxt) {
    ...
    const nuxtUiComponentsDir = dirname(
      consumerRequire.resolve("@nuxt/ui/components/Button.vue"),
    );
    ...
    for (const domain of publicRuntimeDomains)
      nuxt.options.alias[`#ui-tools/${domain}`] = resolve(
        `./runtime/${domain}`,
      );
    nuxt.options.alias["@nuxt/ui/components"] ??= nuxtUiComponentsDir;
    nuxt.options.alias["@nuxt/ui/composables"] ??= join(
      nuxtUiRuntimeDir,
      "composables",
    );
...
function mergeViewportOptions(
  ...
  return {
    ...
    fallbackBreakpoint:
      viewportOptions?.fallbackBreakpoint ??
      viewportDefaults.fallbackBreakpoint,
